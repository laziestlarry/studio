...
const RankBusinessOpportunitiesInputSchema = z.object({
  ...
  focus: z.string().describe('The primary focus for ranking, e.g., full-stack dominance or a specific business model.'),
});
...
const RankedOpportunitySchema = OpportunitySchema.extend({
    ...
    rationale: z.string().describe('The rationale for the assigned rank.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert business analyst. Given a list of business opportunities and a strategic focus, rank them in order of preference. The ranking should be based on which opportunities best align with the stated focus.

Provide a clear rationale for each rank. The output should be a ranked list of the same opportunities provided in the input, with additional 'rank' and 'rationale' fields.

Strategic Focus: {{{focus}}}

Business Opportunities:
{{#each opportunities}}
- {{opportunityName}}: {{description}} (Potential: {{potential}}, Risk: {{risk}}, Quick Return: {{quickReturn}}, Priority: {{priority}})
{{/each}}
`,
});
